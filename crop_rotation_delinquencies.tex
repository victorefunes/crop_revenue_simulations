...
\textbf{Third, the rotation effect is robust to a placebo test designed to isolate the agronomic channel from the price channel}. A counterfactual that raises the year-2 soybean-corn price ratio by 25 percent produces a measurable improvement in delinquency outcomes for the strict-alternation plan, which plants soy in year 2, but produces null effects for continuous corn and for the 2-corn / 1-soy plan, which both lack soy in year 2. The null results validate the cash-flow mechanism: the channel runs through the timing of when soy revenue can retire rollover debt, not through any aggregate price effect that should appear in all three plans.

The empirical contribution sits in this constellation of findings. The methodological contribution is the framework that produces them: a yield generator that conditions nonparametrically on the full six-year rotation state, a credit module that preserves the within-year sequence of origination, accrual, repayment, and rollover (rather than collapsing the year into a single net cash flow), and a price–yield copula that distinguishes field-level from aggregate correlation. None of these components is individually new; they draw on \cite{Burt1963} and \cite{Livingston2015} for the dynamic-state structure, on \cite{Featherstone2006, IfftKuethe2015} for the operating-credit calibration, and on \cite{Ramsey2019} for the copula specification; but their combination produces a previously unavailable object: a multi-year, rotation-conditional default probability that can be compared across plans, regions, and productivity zones.
...
The \textbf{rotation-yield literature} establishes the agronomic facts. \cite{Bullock1992} survey identifies the carry-over effects from prior cropping — corn-after-corn yield penalties on the order of 5–15 percent, soybean nitrogen credits in the corn-after-soy direction, and rotation-complexity gains from longer cycles. \cite{Seifert2017} estimate the corn-after-corn penalty empirically using satellite-derived yields, providing the field-level evidence on which conditional yield distributions can be built. More recent work pushes this strand beyond mean yields into the distribution of outcomes. \cite{Bowles2020} synthesize 347 site-years across 11 long-term experiments in North America and show that diversified rotations reduce corn yield losses by 14–89 percent in putative drought years — years they identify using county-level crop-insurance indemnity data — while also lowering the probability of yields below the 10th percentile at 8 of 11 sites. \cite{Smith2021} extend the same approach to 32 long-term European and North American experiments covering 957 site-years and document a temporal feature: yield benefits from rotation diversification accelerate over time, with the maximum gain accruing decades after adoption. These papers establish that rotation provides not just a mean-yield bump but also a reduction in yield-tail risk that compounds over the planning horizon. Yet the empirical adoption pattern runs counter to this: \cite{socolar2021}, using a field-level rotational complexity index applied to 1.5 million Midwestern fields, documents a century-long simplification trend that is strongest on the most productive land and reinforced by proximity to corn-demanding policy infrastructure. The puzzle that motivates this paper is therefore not whether rotation has benefits — the agronomic literature settles that — but why those documented benefits are insufficient to halt simplification, and whether a financial benefit visible to lenders and insurers could change the calculus.
...
\subsection{\label{subsec:relation}Relation to the simulation}
The framework clarifies what the simulation of Section \ref{sec:data} does and does not do. It evaluates the probability of delinquency — a key component of the objective — for three fixed rotation plans, treating $\lambda$ and $u$ as implicit. It does not solve for $r^*$ or estimate structural preference parameters. This is the appropriate scope for a paper whose primary audience is lenders and insurers, who need the conditional-on-plan delinquency probability rather than the farmer's optimal plan. The framework nonetheless provides welfare grounding that connects the simulation output to the farmer's problem: by Propositions 1 and 2, a lower delinquency probability under rotation translates into strictly higher expected utility for any $\lambda > 0$ and any concave $u$, and the gain is larger than a revenue-only calculation would suggest.
...
\textbf{Agronomic mechanism}: The yield penalty from continuous corn is not symmetric around the mean — it is concentrated in stress years. Two mechanisms produce this asymmetry. First, rootworm pressure accumulates under continuous corn, causing root lodging and standability failures precisely in drought years, when plants are already water-stressed and least able to compensate for root damage. Second, the soil pathogen and pest cycle that the soybean break interrupts operates nonlinearly: at low
pest densities the yield loss is modest, but at high densities — which build up over multiple consecutive corn years — losses are severe and correlated with other stressors. The rotation benefit is therefore largest exactly when conditions are worst, generating the asymmetry that (LTD) formalizes.

This mechanism implies that $\bar{y}(c,z)$ should be interpreted as approximately the stress-year yield level — the point below which the rootworm and pathogen effects become first-order. In the Illinois panel, the 2012 drought provides the sharpest identification of this threshold: it was the most severe single-year stress event in the 2008–2022 window and the year in which the rotation yield advantage was largest in absolute terms. \cite{Bowles2020} document a 14–89 percent reduction in corn yield losses in drought and high-pest-pressure years across 11 long-term experiments, compared with 5–8 percent in normal years, which is
consistent with (LTD) holding well into the left tail while the two distributions converge in the right tail.

\textbf{Empirical support in the Illinois panel}: The nonparametric yield distributions estimated in Section \ref{subsec:prices_costs} allow direct verification of (LTD) within each $(c,z)$ cell. Comparing the empirical CDFs of CSCSCS and CCCCCC at the 10th and 25th percentiles, the dominance condition holds in all 9 cells at the 10th percentile and in all 9 cells at the 25th percentile — with no exceptions across region or productivity zone.\footnote{The uniform result reflects the severity of stress embedded in the 15-year Illinois panel, which includes the 2012 drought. Even in southern low-productivity cells, where the continuous-corn yield penalty is smallest in normal years, the stress-year tail separation is sufficient for (LTD) to hold. Section 5.1 discusses the cross-region pattern in detail.} Figure \ref{fig:ltd_cdf_comparison} plots the two CDFs for the central-high cell — the cell with the largest rotation effect and the sharpest left-tail separation — and illustrates the single crossing in the upper tail that is the hallmark of lower-tail dominance. The CS curve lies uniformly below the CC curve to the left of the delinquency threshold $\bar{y} = 138$ bu/ac, and the curves cross only once, near 205 bu/ac, well above the stress-yield region that drives the credit mechanism.
...
\begin{theorem}{Lower-tail dominance and the rotation utility premium}
    Let $\bar{y}(c,z)$ be the yield level below which operating-loan delinquency occurs under plan CCCCCC in cell $(c,z)$, i.e.\ $\bar{y}(c,z) = \{y : P_t y < C_t(1+i)^{9/12}\}$ at median price. Suppose (LTD) holds at $\bar{y}(c,z)$. Then for any $\lambda > 0$ and any $u$ with $u'' < 0$,
    ...
    \end{remark}
    \begin{proof}
    Decompose the expectation into the region $Y \leq \bar{y}$ and $Y > \bar{y}$. In the region $Y > \bar{y}$, delinquency does not occur under either plan, so the $\lambda$ terms vanish and the comparison reduces to the expected utility of $W_T$. By (LTD), $F_{\text{CS}}$ places weakly less mass in $Y \leq \bar{y}$ than $F_{\text{CC}}$, so $\mathbb{E}[u(W_T(\text{CS}))] \geq \mathbb{E}[u(W_T(\text{CC}))]$ by the standard result that lower-tail dominance is sufficient for expected utility ranking under concave $u$ when the utility function is sufficiently steep in the dominated region. In the region $Y \leq \bar{y}$, delinquency occurs, contributing $-\lambda$ to the objective. By (LTD), $\Pr[B_t(\text{CS}) > 0] \leq \Pr[B_t(\text{CC}) > 0]$ for each $t$, so the $\lambda$ penalty is weakly smaller under CSCSCS. Both components of the objective are weakly larger under CSCSCS, establishing the inequality. Strictness follows from $\Pr[Y \leq \bar{y}] > 0$ under CCCCCC, which ensures the $\lambda$ terms differ. $\square$
    \end{proof}
\end{theorem}

Three features of this result deserve emphasis. First, the dominance condition (LTD) is set at $\bar{y}(c,z)$, the \textit{delinquency threshold}, rather than an arbitrary quantile. This alignment is not a coincidence: it is precisely the region where the farmer's utility is most sensitive to the distributional difference, because the $\lambda$ penalty is there. The utility result therefore does not require (LTD) to hold throughout the left tail — it requires it to hold at the point that matters most for the credit outcome.
...
Proposition 1 establishes that rotation dominates continuous corn in expected utility whenever (LTD) holds. Proposition 2 establishes that the \textbf{magnitude} of this advantage is understated by a calculation that ignores the operating-credit recursion.
...
Field boundaries are defined as contiguous CDL pixels of the same crop in a given year, aggregated to a stable spatial unit using the Common Land Unit (CLU) layer \citep{Yan2016}. Six-year rotation histories are constructed by intersecting each field's geometry with the CDL layer for each of the preceding five years. Histories that overlap multiple CDL classes by more than a small threshold dropped to preserve a clean state definition. The resulting panel contains field-year observations with $(s, c, z, Y)$ as the primitive record, where $s$ is the six-year rotation state, $c$ is the climate region, $z$ is the productivity zone, and $Y$ is the QDANN-estimated yield.

The three climate regions (northern, central, southern Illinois) follow the standard partition used by the \textbf{Illinois Farm Business Farm Management}\footnote{\url{https://www.fbfm.org/}} and the University of Illinois' \textbf{Farmdoc}\footnote{\url{https://farmdoc.illinois.edu/}}; the three productivity zones (high, medium, low) are constructed by partitioning the NCCPI soil productivity index from gSSURGO into terciles. The full grid is $3 \times 3 = 9$ cells, each containing the three rotation plans of interest. Within each cell, the panel contains hundreds of thousands to thousands of field-year observations across the fifteen-year window, sufficient to estimate the conditional yield distribution nonparametrically in all but the rarest rotation states.
...
The simulation has three components: a rotation-conditioned yield generator, a price–yield dependence structure, and an operating-credit cash-flow recursion that together produce a multi-year delinquency probability for any given (field, rotation plan) pair. Each component is itself standard in the literature; the contribution lies in their combination and in the recursion's preservation of within-year credit flow, which converts a single yield shock into multi-year default risk. This section specifies the three components and the outcome measures derived from the simulation. Calibration values and computational details are reported in Appendix \ref{sec:appendix_A}.

\subsection{\label{subsec:overview}Overview}
For each cell $c = (\text{region} \times \text{zone})$, rotation plan $r$, and six-year history $h$, the simulation draws $N = 10{,}000$ Monte Carlo paths over a six-year horizon. A path consists of six (price, yield) realizations passed through the credit recursion to produce a sequence of year-end debt balances.
...
Let $P_t$ denote the harvest-time market price of the crop planted in year $t$. Aggregate U.S. corn prices and yields are negatively correlated at $\hat\rho \approx -0.45$ over the sample period (high national yields depress prices; low national yields raise them), and this correlation generates a "natural hedge" in revenue. The methodological question is how much of that hedge applies at the \textbf{field} level.

The aggregate correlation is between the national price and the national average yield. A field's yield is the sum of two components,

\begin{equation}
    Y_t = \underbrace{Y^{\text{agg}}_t}_{\text{aggregate}} + \underbrace{\varepsilon_t}_{\text{idiosyncratic}},
\end{equation}

where $Y^{\text{agg}}_t$ is the national mean (the only component that moves the national price) and $\varepsilon_t$ captures local soil, hail, management, and field-specific weather. Decomposing the field-level yield panel into a year fixed effect and a field-by-year residual, we obtain $\text{Var}(Y^{\text{agg}}_t) / \text{Var}(Y_t) \approx 0.40$ and the complementary $\text{Var}(\varepsilon_t) / \text{Var}(Y_t) \approx 0.60$. Only the 40-percent aggregate piece correlates with price; the 60-percent idiosyncratic piece does not. Treating the aggregate $\rho$ as if it applied to the entire field-level yield shock would therefore \textbf{overstate} the natural hedge.

We implement the dependence structure via a Gaussian copula on the marginals $F_P$ and $F_{Y \mid s, c, z}$, with the copula correlation parameter set  $\rho^* = \hat\rho \cdot \sqrt{\text{Var}(Y^{\text{agg}})/\text{Var}(Y)} \approx -0.45 \cdot \sqrt{0.40} \approx -0.28$. This produces a price–yield correlation at the field level that reproduces the aggregate $\hat\rho$ when yields are aggregated to the national level, while leaving the empirical marginals untouched.

For comparison, we also report results under two alternative dependence specifications: an independence assumption ($\rho^{*} = 0$), used to isolate the hedge benefit (Section \ref{subsec:natural_hedge}), and the aggregate-$\rho$ misspecification ($\rho^{*} = \hat\rho = -0.45$ applied directly to field yields), which overstates the hedge. Across all cells, the copula approach falls between these two, closer to independence than to the aggregate-$\rho$ assumption, and the delinquency probabilities differ by 1–3 percentage points across the three specifications. This is the magnitude of the natural-hedge effect at the field level.
...
Following \cite{Shorrocks2013} and standard practice in inequality and risk decomposition, the Shapley value for source $s \in \{P, Y\}$ is the  marginal contribution of $s$ across all orderings of adding sources to the simulation:
...
Where $V(\mathcal{A})$ is the delinquency probability when sources in $\mathcal{A}$ are stochastic, and sources outside $\mathcal{A}$ are fixed at their unconditional means. The Shapley values are guaranteed to sum to $V(\{P,Y\}) - V(\emptyset)$, the total delinquency-causing variation, with no interaction-term residual. The decomposition therefore answers a different question than the variance decomposition: not "what share of revenue variation is from price?" but "what share of the \textbf{crossing of the default threshold} is from price?".

Finally, we report a \textbf{placebo specification} (Section \ref{subsec:placebo}) in which the year-2 soybean–corn price ratio is shocked by $+25$ percent and held at that level for one year only. The strict-alternation plan plants soybeans in year 2 and should respond; the continuous-corn and 2-corn/1-soy plans plant no soybeans in year 2 and should not. The null effect on the latter two plans validates the cash-flow mechanism: the channel runs through the \textit{timing} of when soy revenue retires rollover debt, not through any aggregate price effect that would appear in all three plans.

\section{\label{sec:results}Results}
The simulation produces three classes of results. Section \ref{subsec:delinq_prob} presents the headline six-year delinquency probability across the rotation-plan $\times$ region $\times$ zone grid. Section \ref{subsec:sawtooth} unpacks this into the per-year hazard that reveals the sawtooth debt-reset mechanism. Section \ref{subsec:peak_demand} examines the distribution of peak operating-loan demand against representative credit ceilings. Section \ref{subsec:natural_hedge} isolates the natural hedge by comparing simulations with correlated and independent price–yield draws.

\subsection{\label{subsec:delinq_prob}Six-year delinquency probabilities}

Figure \ref{fig:fig_p_bar_all} reports the probability of at least one operating-loan delinquency event over the six-year horizon for each combination of rotation plan, climate region, and productivity zone. Three patterns are immediately apparent.
...
First, continuous corn carries the highest delinquency risk across all cells. In central Illinois's high-productivity zone — typically considered prime corn-growing land — continuous corn produces a six-year delinquency probability of approximately 55 percent. The probability is in the 30–50 percent range for the northern and central regions across all productivity zones, falling to 18–32 percent in the southern region, where lower expected yields are partially offset by lower production costs and a less corn-specialized cost structure.

Second, strict alternation reduces the probability by approximately 25 percentage points in the high-pressure cells. The alternation probability for central-high is approximately 30 percent, for northern-high approximately 27 percent, and the corresponding reductions relative to continuous corn are 25 and 24 percentage points. This is the quantitative magnitude of the rotation effect: not a marginal adjustment, but a halving of multi-year default risk in the cells where corn-only systems are most stressed.

Third, the 2-corn/1-soy plan falls between continuous corn and strict alternation, with delinquency probabilities of approximately 35–40 percent in the high-pressure cells. The ordering is consistent with the agronomic intuition that more soybean years in the rotation provide more debt-reset opportunities, but the relationship is not strictly linear in soybean frequency, as Section \ref{subsec:sawtooth} will show.

The cross-region pattern is consistent with what the agronomic literature predicts: high-pressure rotations exhibit larger plan-to-plan differences than low-pressure rotations. In southern Illinois, where the continuous-corn yield penalty is smaller and production costs are lower, the cross-plan range narrows but does not collapse: the strict-alternation probability remains 10 percentage points below continuous corn in the southern-high cell.

\subsection{\label{subsec:sawtooth}Per-year hazard: the sawtooth}

The cumulative six-year probability is the integral of an underlying per-year hazard. Figure \ref{fig:fig_p_year_all} plots the year-by-year delinquency probability for each plan in the central-medium cell; the patterns in the other eight cells are qualitatively identical, with only the levels differing.
...
The central finding of the paper appears here. Continuous corn produces a flat hazard at roughly 12–15 percent per year over the six-year horizon, with a mild upward trend as accumulated debt increases. The two rotation plans produce a \textbf{sawtooth}: hazard drops to near zero in soybean-harvest years and rebuilds during corn years. Under strict alternation (soybean in years 2, 4, 6), the hazard is near-zero in even years and approximately 11 percent in odd years. Under 2-corn / 1-soy (soybean in years 3 and 6), the hazard is near-zero in years 3 and 6 and elevated in the corn years between.

The mechanism is the debt reset described in Section \ref{subsec:recursion}. A soybean harvest arrives on a smaller spring loan (soybean cost is roughly 70 percent of corn cost in the FBFM data), and retires the accumulated rollover balance from the prior corn years, and resets the carried-debt state to near zero before the next corn run. The cumulative six-year delinquency probability is therefore not a smoothing of risk — it is a \textbf{periodic arrest} of the rollover process that would otherwise convert a single bad year into a multi-year default cascade.

This finding has a substantive implication for the agronomic literature. \cite{Bullock1992} and \cite{Seifert2017} document a yield benefit from rotation; the financial benefit identified here is partially \textbf{independent} of the yield benefit. Even in the simulation where the rotation-conditional yield distributions are held constant - i.e., the corn yield in a rotation plan is the same as the corn yield in continuous corn, year-by-year - the rotation plan still cuts delinquency, because the \textbf{soybean year itself} is a debt reset regardless of how it affects subsequent corn yields. The agronomic and financial channels operate in parallel and add up, which is why the rotation effect is large even in cells where the corn-after-corn yield penalty is modest.
...
\subsection{\label{subsec:placebo}Placebo: the year-2 soybean-price shock}
A natural concern with the rotation-mechanism interpretation in Section \ref{subsec:sawtooth} is that the result could be driven by any feature of the simulation that varies across plans, not specifically by the soybean-year debt reset. To isolate the debt-timing channel, we run a placebo specification: in year 2 only, the soybean–corn price ratio is shocked by $+25$ percent and held there; all other years and all other moments of the price distribution are unchanged from the baseline.

The strict-alternation plan plants soybeans in year 2, so the year-2 soybean revenue rises by 25 percent under the shock. The continuous-corn and 2-corn/1-soy plans plant no soybeans in year 2, so neither sees any change in year-2 revenue. If the rotation effect documented in Section \ref{subsec:sawtooth} operates through the debt-timing channel, the shock should improve delinquency outcomes for strict alternation and have no effect on the other two plans. If, instead, the rotation effect operates through another channel (an aggregate price effect, a yield-distribution artifact, or a calibration anomaly), the shock could, in principle, affect all three plans.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{figs/fig_price_sensitivity_soy_yr2.png}
    \caption{Year-2 soy price shock moves only the soy-in-yr-2 plan}
    \label{fig:fig_price_sensitivity_soy_yr2} 
\end{figure}

Figure \ref{fig:fig_price_sensitivity_soy_yr2} reports the change in six-year delinquency probability under the shock relative to the baseline, by plan, region, and zone. The result is clean. Continuous corn shows changes in magnitude $\leq 0.4$ percentage points across all cells, well within the Monte Carlo error and consistent with a null effect. The 2-corn / 1-soy plan likewise shows changes of $\leq 0.2$ percentage points. The strict-alternation plan shows changes ranging from $-0.4$ to $+0.4$ percentage points across cells, with a clear central-zone pattern of improvement that disappears in the placebo plans. The null results for continuous corn and 2-corn/1-soy confirm that the channel runs through the timing of soybean revenue being \textbf{available to retire debt}, not through any general price effect that should appear in all plans.

A second robustness exercise reports the simulation under an alternative delinquency rule ("over-limit" rather than end-balance-positive). The absolute levels fall — naturally, since a positive balance below the credit ceiling no longer counts- but the relative ranking of plans and the sawtooth pattern in per-year hazards both survive. Appendix \ref{sec:appendix_B} reports additional robustness exercises on the interest-rate calibration, the accrual window, and a working-capital buffer.
...
For \textbf{insurers}, the multi-year delinquency object identified here is not what Revenue Protection (RP) products price. RP premiums are calibrated on the single-year loss-cost ratio, which approximates $E[\max(0, \text{guarantee} - \text{revenue})]$ from a one-year distribution. The single-year object does not see the rollover channel that converts a year-1 shock into a multi-year default cascade. A corn-heavy producer with a high one-year revenue distribution and a poor multi-year survival distribution may therefore appear cheap to insure on the single-year metric and expensive on the multi-year metric. The pricing implication is that RP premiums calibrated on single-year loss ratios systematically understate the \textit{insurance value} of rotation, since rotation's largest financial benefit accrues specifically to the multi-year delinquency dimension that RP does not see. This is a contribution to the credit-insurance interaction literature \cite{IfftKuethe2015}: the federal crop insurance program may be inducing greater use of debt precisely because the rate structure does not reward the rotation choice that would reduce multi-year ruin.
...
\\\
Aggregate yield variance & $0.40$ & Field-panel variance\\
share $\sigma^2_{\text{agg}}/\sigma^2_Y$ & & decomposition\\
\\
Implied copula correlation $\rho^{*}$ & $-0.28$ & $\hat\rho \cdot \sqrt{0.40}$\\
\\
...
\subsection{\label{subsec:param_robust}Parametric-fit robustness for the yield generator}
...
\subsection{\label{subsec:pv_placebo}Present-value placebo}

Section \ref{subsec:placebo} reports the year-2 soybean price placebo as a probability change: a $+25\%$ soy/corn price shock in year 2 only, with the resulting change in the six-year delinquency probability shown in Figure \ref{fig:fig_price_sensitivity_soy_yr2}. The probability framing is the most direct mapping to the paper's central outcome, but the same exercise can be run in dollar terms.

\begin{figure}[H]
    \centering
    \includegraphics[width=16cm]{figs/fig_price_sensitivity_pv_soy_yr2.png}
    \caption{Change in present-value net cash flow under a $+25\%$ year-2 soybean-corn price ratio shock, relative to the baseline simulation, by rotation plan, climate region, and productivity zone.}
    \label{fig:fig_price_sensitivity_pv_soy_yr2} 
\end{figure}

Figure\ref{fig:fig_price_sensitivity_pv_soy_yr2} reports the change in present-value net cash flow under the same year-2 shock. Continuous corn (CCCCCC) and the 2-corn / 1-soy plan (CCSCCS), neither of which plants soy in year 2, show PV changes of \$-6 to \$+6 per acre — within Monte Carlo noise and consistent with a null effect. The strict-alternation plan (CSCSCS), which plants soy in year 2, picks up \$121–\$128 per acre in present value across all cells. The dollar magnitudes confirm the probability pattern: the channel runs through the timing of when soybean revenue is available to retire debt, not through any aggregate price effect.
