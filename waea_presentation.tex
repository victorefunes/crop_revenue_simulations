...
\begin{frame}{Research questions}
    \begin{wideitemize}
        \item \textcolor{red}{\textbf{Question 1}}: How can we estimate delinquency probabilities conditioning on yields on the full six-year rotation history of the field? 

        If revenue is treated as one aggregate stream and debt default is predicted from one-year loss probabilities, crop rotation effects are not accounted for.
        \item \textcolor{red}{\textbf{Question 2}}: How does the negative price–yield correlation change those probabilities, and do its effects act as a substitute for or complement to rotation?
        
        \item \textbf{Sustainable practices}: We show that, crop rotation reduces the debt-rollover chain, and lowers multi-year delinquency risk. It is the link between agronomic sustainability and finncial resilience.
    \end{wideitemize}
\end{frame}
...
\footnotesize
  \begin{itemize}
  \item The rotation effect is large and monotone everywhere.
  \item Northern and Central IL show the largest impact (perfect rotation cuts delinquencies by 30\%), Southern IL has a lower absolute reduction, but a similar relative reduction.
  \item Productivity zone barely matters: it is not related to soil quality.
...
\begin{frame}{Rotation benefits}
    \begin{columns}[T] % align columns
   \begin{column}{.55\textwidth}
    \resizebox{\textwidth}{!}{
        \includegraphics{figs/corn_rot_plot.png}
        }
    \textbf{Note:} Coefficients of a linear regression of corn yields on rotation patterns. Base level is corn monoculture.     
   \end{column}%
   \hfill%
   \begin{column}{.45\textwidth}
    \begin{itemize}
    \item Rotating crops brings a series of benefits in terms of soil health (soybeans fix atmospheric nitrogen, break pest and disease cycles)
    \item We show that rotation also has financial benefits, including a lower probability of farm delinquencies and risk reduction.
    \item Rotating farms benefit less from the natural hedge provided by the negative price-yield correlation because rotation has already done the work the hedge would otherwise do.
    \item $\hat{\beta}_{CCSCCS}=1.18$, $\hat{\beta}_{CSCSCS} = 3.1$
  \end{itemize}
\end{column}%
\end{columns}  
\end{frame}
...
\begin{column}{.4\textwidth}
  ...
  \item The "debt-rest" occurs because:
  \begin{enumerate}
      \item Soybeans have lower spring operating costs than corn.
      \item Soybean harvest revenue is sufficient to clear that lower hurdle under typical conditions.
      \item The credit recursion has no smoothing; it has a discrete repayment event
  \end{enumerate}
...
\begin{frame}{Effects of yield shocks}
    \begin{columns}[T] 
        \begin{column}{.6\textwidth}
            \resizebox{1.05\textwidth}{!}{
              \includegraphics{figs/fig_yield_compare_corr_vs_indep.png}
            }
    \end{column}%
        \hfill%
        \footnotesize
    \begin{column}{.4\textwidth}
        \begin{itemize}
          \item Experiment: $-12\%$ corn yield shock in year 3 only. Reported as change vs. baseline. 
          \item Corresponds to the effect of an approximate 2 hPa average drop in VPD \cite[Chapter~1]{Pizzo2025}
          \item Two rows per panel:  \textbf{correlated prices} (natural hedge active) vs. \textbf{independent prices} (no hedge).
         \item CCSCCS: null control: plants soy in yr 3 $\rightarrow$ corn shock doesn't hit yr-3 revenue $\rightarrow \approx 0$ change expected.
         \item Continuous corn: yield shock compounds
         \item 2 Corn/1 soy: null effect (counterfactual)
         \item Perfect rotation: larger effect than continuous corn
         \item Hedge provides no insulation: Correlated vs. independent rows are nearly identical. Rotation's protection comes from debt-timing — not from hedging yield risk
          \end{itemize}
        \end{column}%
    \end{columns}
\end{frame}
...
\begin{column}{.35\textwidth}
  ...
  \begin{itemize}
    \item The natural hedge is a mitigating factor for the effect of rotation.
    \item The gap is widest for \textcolor{cardinalred}{continuous corn} and nearly closes for rotations.
    \item Hedge and rotation are partial substitutes.
  \end{itemize}
\end{column}%
\end{columns}
\end{frame}
...
\begin{column}{.6\textwidth}
\resizebox{\textwidth}{!}{
  \includegraphics[scale = 1.1]{figs/fig_p_surv_all.png}
}
\end{column}%
\hfill%
\begin{column}{.4\textwidth}
  ...
  \item Continuous corn has no recovery mechanism.
  \item The south row is distinctively different in shape, not just level.
  \item Productivity zone again barely matters.
...
\begin{frame}{Probability of delinquency with price-yield correlation}
\begin{columns}[T] % align columns
\begin{column}{.6\textwidth}
\resizebox{1.1\textwidth}{!}{
  \includegraphics{figs/fig_p_bar_corr.png}
}
\end{column}%
\hfill%
\begin{column}{.4\textwidth}
\footnotesize
  \begin{itemize}
  \item Price-yield correlation dampens the effects of rotations (2.4 pp for continuous corn, 1-2 pp for rotation plans); baseline risk is 30–55\%.
  \item The hedge benefit is real but small everywhere. The hedge is most valuable for continuous corn: rotation has already driven carried debt to near zero in soybean years, when a good national yield depresses price, there is no accumulated rollover balance for the hedge to protect against. 
  \item The southern row is the exception; the faded-to-solid gap for CCCCCC is notably wider (7–9 pp rather than 2–4 pp).
  \item The rotation effect dwarfs the hedge effect. The rotation effect is an order of magnitude larger than the hedge effect.
\end{itemize}
\end{column}%
\end{columns}
\end{frame}
...
\begin{block}{Three things to remember}
    \begin{enumerate}
        ...
        The natural hedge is modest and largely redundant where rotation is in effect.
